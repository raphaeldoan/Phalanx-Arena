...
Phalanx Arena is a deterministic ancient-battle tactics game and LLM benchmark. It supports local browser play, room-code online duels, free browser AI turns through the bundled custom `strategos-1` and `strategos-2` models, player-supplied API keys, and offline AI-vs-AI benchmark runs.
...
Use the `AI Controls` dropdown in the app to assign AI to army `A`, army `B`, or both. For single-player vs AI, `AI Setup` offers two free bundled custom models: `strategos-1`, a hybrid non-LLM policy with a trained action ranker, tactical planner, bounded tactical priors, and 1-ply local engine lookahead; and `strategos-2`, a browser neural policy. Both run locally in the browser and require no API key. BYO-key mode supports `openai`, `anthropic`, `xai`, `mistral`, `gemini`, `together`, and `openrouter`.
...
flowchart LR
  ...
  CLI --> Core
  BrowserAI["Browser AI"] --> LocalAI["strategos-1 / strategos-2"]
  BrowserAI --> Providers["BYO-key providers"]
